...
//return true if attendence entered today
bool isAt_Done(){
...
int ADMIN::admitStudent(){
	...
	clrscr();
	//genusn(temp.usn);
	//strcpy(temp.usn,"1NT13CS065");
	//temp.admissionNo=12339;
...
//initialize the members of staff
...
int STAFF::manageMarks(){
	...
	cout<<"Enter the marks and press enter key : "; cin.get();
	clrscr();
	title();
	cout<<endl<<"Subjcet : ";
		switch(subject){
...
int main(){
    ...
    switch(logChoice) {
		...
		case 4 : cout<<"Terminating..."; Sleep(1000); //from WINAPI Sleep(DWORD)
				exit(0);
        default : cout <<"Invalid choice : "; cin.get();
